import { memo, useState, useCallback } from "react";
import { Button } from "@/components/ui/button";
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogDescription } from "@/components/ui/dialog";

const MobileAppSection = memo(() => {
  const [isAppModalOpen, setIsAppModalOpen] = useState(false);

  const handleModalOpen = useCallback(() => setIsAppModalOpen(true), []);
  const handleModalClose = useCallback(() => setIsAppModalOpen(false), []);

  return (
    <>
      <section className="py-20 bg-[#0C0C0F] relative overflow-hidden">
        <div className="absolute inset-0 opacity-20">
          <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-96 h-96 bg-gradient-to-r from-cyan-400 to-purple-500 rounded-full blur-3xl"></div>
        </div>
        
        <div className="container mx-auto px-6 max-w-6xl relative z-10">
          <div className="flex flex-col md:flex-row items-center justify-between gap-12">
            {/* Mobile layout */}
            <div className="w-full text-center md:hidden mb-4">
              <h2 className="text-3xl font-bold text-white mb-4" style={{ textShadow: '0 0 4px #8F00FF88' }}>
                Cocktailo — скоро в твоём кармане
              </h2>
              <p className="text-zinc-400 text-base">
                Создавай, сохраняй, делись на ходу
              </p>
            </div>

            <div className="w-full flex justify-center md:hidden mb-4">
              <div className="relative">
                <img 
                  src="/attached_assets/ChatGPT Image 18 июл. 2025 г., 23_50_01-Photoroom_1752872369395.png"
                  alt="Cocktailo mobile app"
                  className="w-96 h-auto object-contain hover:scale-105 transition-all duration-300 relative z-10"
                  loading="lazy"
                  width="384"
                  height="768"
                />
                <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-80 h-80 bg-gradient-to-r from-cyan-400/15 to-purple-500/15 rounded-full blur-3xl -z-10"></div>
              </div>
            </div>

            <div className="w-full flex justify-center md:hidden">
              <div className="flex flex-col gap-4 w-full max-w-sm">
                <Button 
                  onClick={handleModalOpen}
                  className="inline-flex items-center justify-center px-5 py-3 rounded-xl bg-white text-black font-semibold hover:scale-105 transition-all duration-300"
                >
                  <svg className="w-6 h-6 mr-2 flex-shrink-0" viewBox="0 0 24 24" fill="currentColor">
                    <path d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.81-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M13 3.5c.73-.83 1.94-1.46 2.94-1.5.13 1.17-.34 2.35-1.04 3.19-.69.85-1.83 1.51-2.95 1.42-.15-1.15.41-2.35 1.05-3.11z"/>
                  </svg>
                  App Store
                </Button>
                <Button 
                  onClick={handleModalOpen}
                  className="inline-flex items-center justify-center px-5 py-3 rounded-xl bg-white text-black font-semibold hover:scale-105 transition-all duration-300"
                >
                  <svg className="w-6 h-6 mr-2 flex-shrink-0" viewBox="0 0 24 24" fill="currentColor">
                    <path d="M3,20.5V3.5C3,2.91 3.34,2.39 3.84,2.15L13.69,12L3.84,21.85C3.34,21.6 3,21.09 3,20.5M16.81,15.12L6.05,21.34L14.54,12.85L16.81,15.12M20.16,10.81C20.5,11.08 20.75,11.5 20.75,12C20.75,12.5 20.53,12.9 20.18,13.18L17.89,14.5L15.39,12L17.89,9.5L20.16,10.81M6.05,2.66L16.81,8.88L14.54,11.15L6.05,2.66Z"/>
                  </svg>
                  Google Play
                </Button>
              </div>
            </div>

            {/* Desktop layout */}
            <div className="hidden md:flex flex-1 justify-start ml-16">
              <div className="relative">
                <img 
                  src="/attached_assets/ChatGPT Image 18 июл. 2025 г., 23_50_01-Photoroom_1752872369395.png"
                  alt="Cocktailo mobile app"
                  className="w-96 h-auto object-contain hover:scale-105 transition-all duration-300 relative z-10"
                  loading="lazy"
                  width="384"
                  height="768"
                />
                <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-80 h-80 bg-gradient-to-r from-cyan-400/15 to-purple-500/15 rounded-full blur-3xl -z-10"></div>
              </div>
            </div>

            <div className="hidden md:flex flex-1 text-left">
              <div>
                <h2 className="text-4xl font-bold text-white mb-4" style={{ textShadow: '0 0 4px #8F00FF88' }}>
                  Cocktailo — скоро в твоём кармане
                </h2>
                <p className="text-zinc-400 text-base mt-2 mb-16">
                  Создавай, сохраняй, делись на ходу
                </p>
                
                <div className="flex flex-row gap-4">
                  <Button 
                    onClick={handleModalOpen}
                    className="inline-flex items-center px-5 py-3 rounded-xl bg-white text-black font-semibold hover:scale-105 transition-all duration-300"
                  >
                    <svg className="w-6 h-6 mr-2 flex-shrink-0" viewBox="0 0 24 24" fill="currentColor">
                      <path d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.81-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M13 3.5c.73-.83 1.94-1.46 2.94-1.5.13 1.17-.34 2.35-1.04 3.19-.69.85-1.83 1.51-2.95 1.42-.15-1.15.41-2.35 1.05-3.11z"/>
                    </svg>
                    App Store
                  </Button>
                  <Button 
                    onClick={handleModalOpen}
                    className="inline-flex items-center px-5 py-3 rounded-xl bg-white text-black font-semibold hover:scale-105 transition-all duration-300"
                  >
                    <svg className="w-6 h-6 mr-2 flex-shrink-0" viewBox="0 0 24 24" fill="currentColor">
                      <path d="M3,20.5V3.5C3,2.91 3.34,2.39 3.84,2.15L13.69,12L3.84,21.85C3.34,21.6 3,21.09 3,20.5M16.81,15.12L6.05,21.34L14.54,12.85L16.81,15.12M20.16,10.81C20.5,11.08 20.75,11.5 20.75,12C20.75,12.5 20.53,12.9 20.18,13.18L17.89,14.5L15.39,12L17.89,9.5L20.16,10.81M6.05,2.66L16.81,8.88L14.54,11.15L6.05,2.66Z"/>
                    </svg>
                    Google Play
                  </Button>
                </div>
              </div>
            </div>
          </div>
        </div>
      </section>

      <Dialog open={isAppModalOpen} onOpenChange={handleModalClose}>
        <DialogContent className="bg-gradient-to-br from-night-blue to-graphite border-2 border-neon-turquoise/30 max-w-md">
          <DialogHeader>
            <DialogTitle className="text-2xl font-bold text-neon-turquoise">Скоро...</DialogTitle>
            <DialogDescription className="text-zinc text-base pt-4">
              Мобильное приложение Cocktailo находится в разработке. Подпишитесь на рассылку, чтобы узнать первым о запуске!
            </DialogDescription>
          </DialogHeader>
        </DialogContent>
      </Dialog>
    </>
  );
});

MobileAppSection.displayName = "MobileAppSection";

export default MobileAppSection;
